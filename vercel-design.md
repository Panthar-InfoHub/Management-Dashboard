...
Design and develop a world-class internal Team Operations Platform with a premium user experience inspired by the design language and usability of companies like Vercel, Linear, GitHub, Notion, Stripe, Raycast, Clerk, Supabase, and Figma.

This is not just a project management tool—it should be the central operating system for an entire company, where every aspect of engineering, product development, team collaboration, reporting, and daily operations is managed from one place.

The interface should feel modern, minimal, fast, elegant, highly interactive, and information-dense without feeling cluttered.
...
Everything should be built using static mock data so the entire application feels fully functional.

After the UI is finalized, we will integrate:
...
Next.js (App Router)
Prisma ORM (future integration only)
Clerk Authentication (future integration only)

No backend implementation is required in this phase.

Design Philosophy

The application should resemble something built by Vercel's design team.
...
The UI should look like an expensive SaaS product rather than a generic admin dashboard.
...
Instead of basic cards, create a highly interactive executive dashboard.
...
Show AI-style insights (static content for now).

Examples

Backend team completed 96% of sprint goals.

Three projects are approaching deadlines this week.

Five employees haven't submitted today's update.

AI Team workload is 24% higher than average.

Frontend productivity increased by 17%.

These should appear as premium insight cards.
...
Include avatars and timestamps.
...
Beautiful drag-and-drop style layout.
...
Modern roadmap similar to Linear.
...
Tasks should feel similar to Linear.
...
Daily Update Streak
Attendance Score
Daily Update System (Core Feature)

This should be one of the most polished modules in the application.

Every user sees a large, prominent "Submit Daily Update" action on their dashboard.

The submission experience should be fast, frictionless, and visually appealing.
...
A dedicated company-wide page that acts like an internal social timeline.

Features:

Timeline of all submitted updates
Rich cards with avatars
Team badges
Project references
Search
Filters
...
Manager dashboards should immediately highlight who has and hasn't submitted updates.
...
An internal knowledge base similar to Notion.
...
Dedicated analytics section with executive-level dashboards.
...
A universal search inspired by Raycast and Linear.
...
Comprehensive settings area with sections for:
...
Create a scalable design system with reusable components including:
...
Modular, reusable page layouts
Realistic mock data throughout to simulate a production-ready application
Expected Outcome

The final deliverable should feel like a polished, enterprise-grade product that could be used internally by a fast-growing technology company. Every page should be thoughtfully designed, interconnected, and populated with realistic static content so stakeholders can fully experience the product before backend development begins. The emphasis is on exceptional UI/UX, intuitive workflows, executive-level dashboards, and a cohesive design system that is ready for seamless integration with Clerk authentication, Prisma ORM, and the application's business logic in the next phase.171717"
